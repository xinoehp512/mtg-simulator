...
human = Creature_Card("Human (W)", Cost.from_string("W"), [CardType.CREATURE], [], "", 1, 1)
merfolk = Creature_Card("Merfolk (U)", Cost.from_string("U"), [CardType.CREATURE], [], "", 1, 1)
goblin = Creature_Card("Goblin (R)", Cost.from_string("R"),  [CardType.CREATURE], [], "", 1, 1)
druid = Creature_Card("Druid (G)", Cost.from_string("G"),  [CardType.CREATURE], [], "", 1, 1)

ox = Creature_Card("Ox (1W)", Cost.from_string("1W"), [CardType.CREATURE], [], "", 2, 2)
crab = Creature_Card("Crab (1U)", Cost.from_string("1U"), [CardType.CREATURE], [], "", 1, 3)
dwarf = Creature_Card("Dwarf (1R)", Cost.from_string("1R"), [CardType.CREATURE], [], "", 3, 1)
bear = Creature_Card("Bear (1G)", Cost.from_string("1G"), [CardType.CREATURE], [], "", 2, 2)

horse = Creature_Card("Horse (2W)", Cost.from_string("2W"), [CardType.CREATURE], [], "", 3, 3)
serpent = Creature_Card("Serpent (2U)", Cost.from_string("2U"), [CardType.CREATURE], [], "", 2, 5)
drake = Creature_Card("Drake (2R)", Cost.from_string("2R"), [CardType.CREATURE], [], "", 4, 2)
ent = Creature_Card("Ent (2G)", Cost.from_string("2G"), [CardType.CREATURE], [], "", 3, 3)

angel = Creature_Card("Angel (3W)", Cost.from_string("3W"), [CardType.CREATURE], [], "", 4, 4)
sphinx = Creature_Card("Sphinx (3U)", Cost.from_string("3U"), [CardType.CREATURE], [], "", 3, 6)
dragon = Creature_Card("Dragon (3R)", Cost.from_string("3R"), [CardType.CREATURE], [], "", 7, 1)
dinosaur = Creature_Card("Dinosaur (3G)", Cost.from_string("3G"), [CardType.CREATURE], [], "", 5, 5)
...
apothecary_stomper = Creature_Card("Apothecary Stomper (4GG)", Cost.from_string(
    "4GG"), [CardType.CREATURE, CreatureType.ELEPHANT], [vigilance, apothecary_stomper_etb], "", 4, 4)
armasaur_guide = Creature_Card("Armasaur Guide (4W)", Cost.from_string(
    "4W"), [CardType.CREATURE, CreatureType.DINOSAUR], [vigilance, armasaur_guide_attack], "", 4, 4)
axgard_cavalry = Creature_Card("Axgard Cavalry (1R)", Cost.from_string(
    "1R"), [CardType.CREATURE, CreatureType.DWARF, CreatureType.BERSERKER], [axgard_cavalry_tap], "", 2, 2)
bake_into_a_pie = Instant_Card("Bake into a Pie (2BB)", Cost.from_string("2BB"), [CardType.INSTANT], [bake_into_a_pie_ability], "")
banishing_light = Enchantment_Card("Banishing Light (2W)", Cost.from_string("2W"), [CardType.ENCHANTMENT], [banishing_light_ability], "")
beast_kin_ranger = Creature_Card("Beast-Kin Ranger (2G)", Cost.from_string("2G"),
                                 [CardType.CREATURE, CreatureType.ELF, CreatureType.RANGER], [trample, beastkin_ranger_pump], "", 3, 3)
bigfin_bouncer = Creature_Card("Bigfin Bouncer (3U)", Cost.from_string(
    "3U"), [CardType.CREATURE, CreatureType.SHARK, CreatureType.PIRATE], [bigfin_bouncer_etb], "", 3, 2)
bite_down = Instant_Card("Bite Down (1G)", Cost.from_string("1G"), [CardType.INSTANT], [bite_down_ability], "")
...
broken_wings = Instant_Card("Broken Wings (2G)", Cost.from_string("2G"), [CardType.INSTANT], [broken_wings_ability], "")
burglar_rat = Creature_Card("Burglar Rat (1B)", Cost.from_string("1B"), [CardType.CREATURE, CreatureType.RAT], [burglar_etb], "", 1, 1)
burst_lightning = Instant_Card("Burst Lightning (R)", Cost.from_string("R"), [CardType.INSTANT], [
    burst_lightning_ability, kicker(Cost.from_string("4", type=AdditionalCostType.KICKED))], "")
bushwhack = Sorcery_Card("Bushwhack (G)", Cost.from_string("G"), [CardType.SORCERY], [bushwhack_ability], "")
cackling_prowler = Creature_Card("Cackling Prowler (3G)", Cost.from_string(
    "3G"), [CardType.CREATURE, CreatureType.HYENA, CreatureType.ROGUE], [ward(Cost.from_string("2")), cackling_prowler_morbid], "", 4, 3)
campus_guide = Creature_Card("Campus Guide (2)", Cost.from_string(
    "2"), [CardType.ARTIFACT, CardType.CREATURE, CreatureType.GOLEM], [campus_guide_etb], "", 2, 1)
cathar_commando = Creature_Card("Cathar Commando (1W)", Cost.from_string(
    "1W"), [CardType.CREATURE, CreatureType.HUMAN, CreatureType.SOLDIER], [flash, cathar_sac], "", 3, 1)
courageous_goblin = Creature_Card("Courageous Goblin (1R)", Cost.from_string(
    "1R"), [CardType.CREATURE, CreatureType.GOBLIN], [courageous_goblin_attack], "", 2, 2)
crackling_cyclops = Creature_Card("Crackling Cyclops (2R)", Cost.from_string(
    "2R"), [CardType.CREATURE, CreatureType.CYCLOPS, CreatureType.WIZARD], [crackling_cyclops_pump], "", 0, 4)
crypt_feaster = Creature_Card("Crypt Feaster (3B)", Cost.from_string(
    "3B"), [CardType.CREATURE, CreatureType.ZOMBIE], [menace, crypt_feaster_threshold], "", 3, 4)
dazzling_angel = Creature_Card("Dazzling Angel (2W)", Cost.from_string(
...
dwynens_elite = Creature_Card("Dwynen's Elite (1G)", Cost.from_string(
